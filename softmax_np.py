...
def normalize(train_x, val_x, test_x):
    ...
    for i in range(m):
        for j in range(r):
            train_mean += train_x[i][j]
    train_mean = train_mean / (m*r)

    for i in range(m):
        for j in range(r):
            train_std += (train_x[i][j] - train_mean) ** 2
...
def test(y_hat, test_y):
    ...
    for i in range(len(classes)):
        for j in range(len(classes)):
            confusion_mat[i, j] = np.sum((test_y == classes[i]) & (y_hat == classes[j]))
